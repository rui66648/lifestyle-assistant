...
> 本文档涵盖 Android 签名包构建、Google Play 上架、国内应用市场上架、以及应用内版本检测更新流程。
...
产物路径：
- APK: `www/app-release.apk`（直接分发 / 国内市场上传）
...
```
同步更新 `www/version.json` 中的 `versionCode` / `versionName` / `whatsNew`。
